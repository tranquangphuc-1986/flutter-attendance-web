...
class ReportFirebaseService {
  ...
  Future<void> updateReport(DailyReport report) async {
    await reportCollection.doc(report.id).update(report.toMap());
  }
  ...
  Stream<List<DailyReport>> getReports() {
    return reportCollection.snapshots().map((snapshot) {
      return snapshot.docs
          .map(
            (doc) =>
            DailyReport.fromMap(doc.id, doc.data() as Map<String, dynamic>),
      )
          .toList();
    });
  }
...
//Lấy danh sách dưới dạng List
  Future<List<DailyReport>> fetchReports() async {
    ...
    return snapshot.docs
        .map((doc) => DailyReport.fromFirestore(doc as DocumentSnapshot<Map<String, dynamic>>))
        .toList();
  }
  ...
  Future<void> markAttendance(String studentId, String status) async {
    ...
    await reportCollection.doc('$today-$studentId').set({
      'studentId': studentId,
      'status': status,
      'date': timestampToday,
    });
