...
class UserModel_Phone {
  ...
  factory UserModel_Phone.fromMap(String id, Map<String, dynamic> map) {
    return UserModel_Phone(
      id: id,
      name: map['name'] ?? '',
      phone: map['phone'] ?? '',
...
//lấy toàn bộ danh sách tài khoản từ collection ('userLogin') trong firestore - dạng list
  factory UserModel_Phone.fromFirestore(DocumentSnapshot<Map<String, dynamic>> doc) {
    final data = doc.data()!;
    return UserModel_Phone(
      id: doc.id,
      name: data['name'],
...
class FirebaseUserService {
  final CollectionReference userCollection =  FirebaseFirestore.instance.collection('user_phone');
  ...
  Future<void> addUser(UserModel_Phone user) async {
    await userCollection.doc(user.uid).set(user.toMap());
  }

  // 🔁 Cập nhật user
  Future<void> updateUser(UserModel_Phone user) async {
  ...
  Stream<List<UserModel_Phone>> getAllUsersStream() {
    return userCollection.snapshots().map((snapshot) {
      return snapshot.docs
          .map((doc) =>
          UserModel_Phone.fromMap(doc.id, doc.data() as Map<String, dynamic>))
          .toList();
    });
  }

  //Lấy danh sách dưới dạng List
  Future<List<UserModel_Phone>> getAllUsersList() async {
    QuerySnapshot snapshot = await userCollection.get();
    return snapshot.docs
        .map((doc) => UserModel_Phone.fromFirestore(doc as DocumentSnapshot<Map<String, dynamic>>))
        .toList();
  }

  // 🔍 Lấy user theo email
  Future<UserModel_Phone?> getUserByEmail(String email) async {
    final snapshot =
    await userCollection.where('email', isEqualTo: email).limit(1).get();
    if (snapshot.docs.isNotEmpty) {
      final doc = snapshot.docs.first;
      return UserModel_Phone.fromMap(doc.id, doc.data() as Map<String, dynamic>);
    }
    return null;
  }
...
class AuthServicePhone {
  ...
  // 🟩 Đăng ký tài khoản bằng số điện thoại + pass
  Future<String> signUpUser(UserModel_Phone user) async {
       try {
     // 1️⃣ Tạo email ảo từ số điện thoại
      final email = _phoneToEmail(user.phone);
    // 2️⃣ Tạo email ảo từ số điện thoại Tạo Auth User
      final userCredential = await _auth.createUserWithEmailAndPassword(
        email: email,
        password: user.password,
      );
      final uid = userCredential.user!.uid;
      // 3️⃣ Tạo user mới có UID
      final newUser = UserModel_Phone(
        id: uid,
        uid: uid,
        name: user.name,
        phone: user.phone,
        className: user.className,
        email: email,          // email ảo
        password: user.password,
        role: user.role,
        score: user.score,
      );
      ...
      return "Firebase Auth lỗi: ${err.message}";
    } catch (err) {
      return err.toString();
     }
  }
  ...
  Future<String> loginUser({
    ...
    try {
      ...
      return "Firebase Auth lỗi: ${e.message}";
    } catch (e) {
      return "Lỗi khác: $e";
    }
  }
